...
export function SearchBox(props: ISearchProps) {
  ...
  const isComposition = React.useState<boolean>(false); // 是否处于输入法状态
  ...
  React.useEffect(() => {
    ...
    // 打印搜索结果
  }, [SearchBoxValue, isComposition.current]);
  ...
  const suffixCom = <div className='suffix-box' onClick={() => onSearch(SearchBoxValue)}>
      <SearchOutlined />
    </div>;
  const sugrecPanel = (data: searchSugrecResultType) => {
    ...
    if (g && typeof g !== 'undefined') {
      return <div className='sugrec-panel-box'>
          <div className='sugrec-panel-content'>
            {g.map((item, index) => {
            return <div key={item.sa} className='sugrec-panel-item' onClick={() => searchMethos(item.q)}>
  ...
  return <>
      <div data-component-searchbox="" className='Component-SearchBox'>
        <div className="Search">
          <Input className='SearchInput' placeholder='请输入搜索内容' type='search' value={SearchBoxValue} onChange={e => inputValueChange(e)} suffix={suffixCom} onPressEnter={onPressEnter} bordered={false} />
