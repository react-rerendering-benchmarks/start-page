...
export default memo(function Setting(props: ISettingProps) {
  // 抽屉菜单，设置选项
  const drawerMenu = [{
    label: '搜索',
    key: 'openType'
  ...
  return <>
      <Drawer title="设置" width={500} bodyStyle={{
      padding: 0
    }} headerStyle={{
      borderBottom: 'none'
    }} onClose={props.drawerOnClose} open={props.drawerVisible}>
        <div className='Components-settings'>
          <div className="menu">
            <Menu style={{
            width: 130,
            borderInlineEnd: 'none'
          }} defaultSelectedKeys={['openType']} mode="inline" items={drawerMenu} onClick={({
            key
