...
const YCalendar = memo(() => {
  ...
  const table = React.useState<IDBObjectStore>();
  // 初始化indexdb
  React.useEffect(() => {
  ...
  return <>
      <div className="page-component-calendar" onClick={yCalendarClick}>
        ...
        fontSize: "50px",
        color: "#fff"
      }}>{dayjs(new Date()).format("DD")}</span>
        <span>日期</span>
      </div>
      <YModal visible={visible} closeClick={useCallback(() => setVisible(false), [setVisible])} className="y-calendar">
        <div className="y-calendar-content">
          <div className="left" style={style}>
            <Calendar fullscreen={false} style={{
            height: '100%',
            borderRadius: "10px",
            padding: "0 10px"
          }} onSelect={date => {
