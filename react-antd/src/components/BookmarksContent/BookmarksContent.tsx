...
const BookmarksContent = memo(() => {
  ...
  const activeKey = React.useState(['收藏夹栏']); // 默认选中
  ...
  const bookmarksData = (data: any) => {
    return <div className='bookmarks-container'>
        ...
        return <div key={item.title}>
                {item.children ? <Collapse defaultActiveKey={defaultActiveKey(item.title)} ghost>
                      <Panel header={item.title} key={item.title} style={{
              borderRadius: 10
            }}>
                        {item.children && bookmarksData(item.children)}
                      </Panel>
                    </Collapse> : <div className='bookmarks-content' onClick={() => openBookmarks(item.url)}>
  ...
  const contentFilter = (con: any) => {
    return <span className='bookmarks' title={con.url}>{con.title}</span>;
  };
  // 打开书签
